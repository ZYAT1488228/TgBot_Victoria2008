...
def stage_1_2(message: types.Message):
    ...
    bot.send_message(
        chat_id=message.chat.id,
        text="Введіть номер паспорта водія:",
        reply_markup=markup
    )
...
def stage_1_3(message: types.Message):
    ...
    users[message.chat.id]['passport_number'] = passport_number
    
    # Продолжаем существующий диалог с вопросом про машину
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    cancel_btn = types.KeyboardButton('Скасувати')
...
def get_pass(message: types.Message):
    ...
    file, name = make_pass(        
        ...
        birth_date = users[message.chat.id]['birth_date'],
        passport_number = users[message.chat.id]['passport_number']
    )
